...
export const app = {
  ...
  matchMediaListener(
    breakpoint,
    callbackLessThan = () => undefined,
    callbackBiggerThan = () => undefined
  ) {
  ...
  sliderSpy(slider, callback = (index) => {}) {
    document
      .querySelectorAll(`${slider} .uk-slider-items > li`)
      .forEach((el, idx) => {
    ...
    window.addEventListener("mousemove", function (e) {
      ...
      document.querySelectorAll(mouseMoveItemSelector).forEach((e) => {
        e.style.transform = `translate(${
          -x * e.getAttribute("data-parallax-k")
        }px, ${-y * e.getAttribute("data-parallax-k")}px)`;
      });
    });
  },
    ...
    document
      ...
      .forEach((el) => {
        el.addEventListener("change", (ev) => {
          ...
          const errors =
            validate(currentForm, this.formConstraints(form)) || {};
...
export const quiz = {
  create() {
    ...
    let items = "";
    document
      .querySelectorAll(`ul.quiz-items > li`)
      .forEach(() => (items += '<li><a href="#"></a></li>'));
    ...
    document
      .querySelector(`ul.quiz-nav`)
      ?.insertAdjacentHTML("afterbegin", items);
    ...
    UIkit.switcher($quizSwitcher, {
      animation:
        "uk-animation-slide-right-small, uk-animation-slide-left-small",
      swiping: false,
    });
    ...
    $quizNav.querySelectorAll(`a`).forEach((el, idx, set) => {
      el.addEventListener("click", (ev) => {
        ev.preventDefault();
        app.changeActivitySet(
          $quizNav.querySelectorAll(`li`),
          idx,
          "quiz-active"
        );
    ...
    $itemSet.forEach((el, key, set) => {
      el.addEventListener("beforeshow", (ev) => {
        ...
        if ($quizProgress) {
          $quizProgress.value = `${
            (100 / ($itemSet.length - 1)) * currentIndex
          }`;
        }
